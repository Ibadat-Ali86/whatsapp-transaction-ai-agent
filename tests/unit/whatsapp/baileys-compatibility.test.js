...
test('loads the pinned Baileys v7 API from the CommonJS application boundary', async () => {
  ...
  const { createConnection, shouldReconnectForStatus } = require('../../../src/whatsapp/connection');
  ...
  assert.equal(shouldReconnectForStatus(baileys.DisconnectReason.connectionLost, baileys.DisconnectReason), true);
  assert.equal(shouldReconnectForStatus(baileys.DisconnectReason.loggedOut, baileys.DisconnectReason), false);
  assert.equal(shouldReconnectForStatus(baileys.DisconnectReason.connectionReplaced, baileys.DisconnectReason), false);
});
